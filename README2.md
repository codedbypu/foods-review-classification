...
| ------------------------ | -------------------------------------------- | --------------------------------------------------------------------------- |
| `RAW_DATA_PATH`          | `data/wongnai/train.csv`                     | ไฟล์เทรนหลัก (แก้ได้ — เปลี่ยนเป็น `data/mock/train.csv` สำหรับ smoke test) |
| `WONGNAI_TRAIN_PATH`     | เหมือน `RAW_DATA_PATH`                       | alias ของ train Wongnai                                                     |
| `WONGNAI_TEST_PATH`      | `data/wongnai/test.csv`                      | Wongnai test (default evaluate / notebook)                                  |
| `MOCK_TEST_PATH`         | `data/mock/test.csv`                         | test จำลอง (ไม่มี generator ใน repo — สร้างเองหรือ copy)                    |
| `MOCK_TRAIN_PATH`        | เหมือน `RAW_DATA_PATH` ในโค้ดปัจจุบัน        | ใช้ใน `generate_mock_data.py` — **ดูหมายเหตุ §6.5**                         |
...
| ตัวแปร                      | ค่าเริ่มต้น | ความหมาย                                                   |
| --------------------------- | ----------- | ---------------------------------------------------------- |
| `XGB_ROUNDS`                | `400`       | จำนวน boosting round สูงสุด                                |
| `XGB_EARLY_STOPPING_ROUNDS` | `25`        | หยุดถ้า val loss ไม่ดีขึ้นติดต่อกัน N รอบ                  |
| `XGB_USE_SAMPLE_WEIGHT`     | `True`      | ใส่ `weight` ใน DMatrix **train** เพื่อแก้ class imbalance |
| `XGB_LOW_STAR_BOOST`        | `1.5`       | คูณ weight เพิ่มสำหรับดาว 1–2 บน balanced weight           |
...
**ข้อมูล train:** default `RAW_DATA_PATH` ชี้ Wongnai train เต็ม — evaluate บน `WONGNAI_TEST_PATH` (`test.csv`) ที่ไม่ได้ใช้เทรน
...
| 7    | บันทึก `artifacts/baseline/tfidf_vectorizer.joblib` และ `artifacts/baseline/xgb_model.json` |
...
1. โหลดจาก `config.TFIDF_VECTORIZER_PATH` + `config.XGB_MODEL_PATH`
...
- ตรวจ artifact ก่อนรัน (`ensure_artifacts`) — แจ้ง path ที่ขาดแล้ว exit 1
- default input: `config.WONGNAI_TEST_PATH` (`data/wongnai/test.csv`)
...
**Score ใน notebook:** ใช้ **XLM-R** เป็นค่า default (ไม่ใช่ baseline) — ปรับ import ใน cell 8 ได้ถ้าต้องการ baseline
...
**คำเตือน:** ในโค้ดปัจจุบันเขียนไปที่ `config.MOCK_TRAIN_PATH` ซึ่ง **alias กับ `RAW_DATA_PATH` (Wongnai train)** — อาจทับไฟล์ Wongnai จริง ควรตั้ง `MOCK_TRAIN_PATH = data/mock/train.csv` ใน `config.py` ก่อนรัน หรือเปลี่ยน `RAW_DATA_PATH` ไป mock หลัง generate
...
| -------------- | --------------------- | ----------------------------------------------------- |
| train mock     | `data/mock/train.csv` | ตั้ง `RAW_DATA_PATH` ชี้ที่นี่ก่อนเทรน                |
| test mock      | `data/mock/test.csv`  | ไม่มี generator — copy/split เอง หรือใช้ Wongnai test |


**ข้อควรระวัง:** `MOCK_TRAIN_PATH` ใน `config.py` ปัจจุบันชี้เดียวกับ `RAW_DATA_PATH` (Wongnai) — ดู §5.9 ก่อนรัน generator
...
| ---------- | --------------------------------------------------------------------------------- |
| `baseline` | `artifacts/baseline/tfidf_vectorizer.joblib`, `artifacts/baseline/xgb_model.json` |
...
| `--save-predictions` | off                                           | `outputs/eval/eval_baseline_preds.csv`, `outputs/eval/eval_xlmr_preds.csv` |
...
**Q: README.md บอก default train เป็น mock แต่ config เป็น Wongnai?**  
A: โค้ดปัจจุบันใช้ `RAW_DATA_PATH = data/wongnai/train.csv` — README2 สะท้อนตามโค้ด; เปลี่ยนเป็น mock ใน `config.py` ได้ตาม §6.5
